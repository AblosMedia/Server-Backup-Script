...
#`echo 'Running backup (without S3 upload) of Stoneleigh databases on #{current_time}' | mail -s 'Running backup (without S3 upload) of Stoneleigh databases on #{current_time}' '#{email}'`
...
d.each {
	...
	success = system("mkdir -p #{output_dir}/#{val['slug']}_database_#{period}; touch #{output_dir}/#{val['slug']}_database_#{period}/keepfile")
	if(!success)
		puts ">>>> Error: Problem creating database current backup directory <<<<"
		`echo 'Error creating current backup directory for #{key}' | mail -s 'Error creating previous backup directory for #{key}' '#{email}'`
		next
	end
	success = system("mkdir -p #{output_dir}/#{val['slug']}_database_previous_#{period}; touch #{output_dir}/#{val['slug']}_database_previous_#{period}/keepfile")
	if(!success)
		puts ">>>> Error: Problem creating database previous backup directory <<<<"
	...
	command = "rm -R #{output_dir}/#{val['slug']}_database_previous_#{period}"
	...
	success = system("mkdir -p #{output_dir}/#{val['slug']}_database_previous_#{period}")
	if(!success)
	...
	## Move files
	command = "mv #{output_dir}/#{val['slug']}_database_#{period}/* #{output_dir}/#{val['slug']}_database_previous_#{period}"
	success = system(command)
	if(!success)
	...
	command = "scp #{val['remote_user']}@#{val['remote_host']}:#{val['remote_dir']}/db/#{filename} #{output_dir}/#{val['slug']}_database_#{period}/#{filename}"
	...
	# Alert user
	puts "✔ Database backup successfully taken (without S3 upload) for #{key} on #{current_time}. Filename: #{filename}"
	#`echo 'Database backup successfully taken (without S3 upload) for #{key} on #{current_time}. Filename: #{filename}' | mail -s 'Database backup successfully taken (without S3 upload) for #{key} on #{current_time}. Filename: #{filename}' '#{email}'`
